...
/*   Updated: 2025/03/10 00:00:30 by sechlahb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void cost(t_list *stackA, t_list *stackB)
{
    ...
    while (stackB)
    {
        if (stackB->pos <=  sizeB / 2 && stackB->target->pos <=  sizeA / 2)
        {
            if (stackB->pos >= stackB->target->pos)
                stackB->cost = stackB->pos;
            else
                stackB->cost = stackB->target->pos;
        }else if (stackB->pos >  sizeB / 2 && stackB->target->pos >  sizeA / 2)
        {
            if ((sizeB - stackB->pos) > (sizeA - stackB->target->pos))
